...
function formatearValor(pregunta: string, valor: any, escalaSatisfaccion: number): string {
  if (detectSatisfaccion(pregunta)) return `${valor} / ${escalaSatisfaccion}`
  return String(valor)
}

export default function ModalRespuestas({ respuesta, escalaSatisfaccion = 7, onCerrar }: ModalRespuestasProps)  {
          ...
          {preguntas.map(([pregunta, valor]) => (
            ...
            >
              ...
              <p className="text-sm font-medium text-slate-700 dark:text-slate-200">
                {formatearValor(pregunta, valor, escalaSatisfaccion)}
              </p>
            </div>
          ))}
        </div>
